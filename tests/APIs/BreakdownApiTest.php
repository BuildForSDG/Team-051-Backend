<?php namespace Tests\APIs;

use Illuminate\Foundation\Testing\WithoutMiddleware;
use Illuminate\Foundation\Testing\DatabaseTransactions;
use Tests\TestCase;
use Tests\ApiTestTrait;
use App\Models\Breakdown;

class BreakdownApiTest extends TestCase
{
    use ApiTestTrait, WithoutMiddleware, DatabaseTransactions;

    /**
     * @test
     */
    public function test_create_breakdown()
    {
        $breakdown = factory(Breakdown::class)->make()->toArray();

        $this->response = $this->json(
            'POST',
            '/api/breakdowns', $breakdown
        );

        $this->assertApiResponse($breakdown);
    }

    /**
     * @test
     */
    public function test_read_breakdown()
    {
        $breakdown = factory(Breakdown::class)->create();

        $this->response = $this->json(
            'GET',
            '/api/breakdowns/'.$breakdown->id
        );

        $this->assertApiResponse($breakdown->toArray());
    }

    /**
     * @test
     */
    public function test_update_breakdown()
    {
        $breakdown = factory(Breakdown::class)->create();
        $editedBreakdown = factory(Breakdown::class)->make()->toArray();

        $this->response = $this->json(
            'PUT',
            '/api/breakdowns/'.$breakdown->id,
            $editedBreakdown
        );

        $this->assertApiResponse($editedBreakdown);
    }

    /**
     * @test
     */
    public function test_delete_breakdown()
    {
        $breakdown = factory(Breakdown::class)->create();

        $this->response = $this->json(
            'DELETE',
             '/api/breakdowns/'.$breakdown->id
         );

        $this->assertApiSuccess();
        $this->response = $this->json(
            'GET',
            '/api/breakdowns/'.$breakdown->id
        );

        $this->response->assertStatus(404);
    }
}
